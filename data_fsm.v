module data_fsm #(
    ...
    // FIFO memory
    reg [DATA_W - 1:0] fifo_mem [0:255];
    reg [7:0]   fifo_wptr;
    reg [7:0]   fifo_rptr;
    integer j;
    reg config_error_size,config_error_src, config_error_des, config_error_trigout, config_error_inc, config_error_x_type, config_error_case3, config_error_case6,config_error_transize;
...
generate
    for (a = 0; a < STRB_W; a = a + 1) begin : GEN_STROBE_MASK
        assign strobe_mask[a*8 +: 8] = (state == W /*&& fifo_rptr == 0*/ && !(WVALID && WREADY))?{8{WSTRB[a]}}:{8{WSTRB_wire[a]}};
    end
endgenerate
    
    assign config_error = config_error_size | config_error_src | config_error_des | config_error_trigout | config_error_inc | config_error_x_type | config_error_case3 | config_error_case6 | config_error_transize;
    ...
    assign WSTRB = (state == W) ? ((1 << (1 << transize)) - 1)<< DESADDR_UPDATED_wire[$clog2(DATA_W/8)-1:0] : 0;
    assign WSTRB_wire = (next_st == W) ? ((1 << (1 << transize)) - 1)<< wire_11[$clog2(DATA_W/8)-1:0] : 0;
    always@(*) begin
        DESADDR_UPDATED_wire = des_addr_reg;
        if(state == W  && des_left !=0)
         begin 
            if(WREADY)
                begin
                    if(des_xaddr_inc == 1)
        ...
        begin
            ...
            begin
                ...
                if(RVALID) begin
                    ...
                    begin
                        SRCADDR_UPDATED <= (case6 && x_type == 2) ? (src_left>(desxsize_reg - srcxsize_reg)) ? (src_addr_reg + ((srcxsize_reg + 1  - (src_left  -(desxsize_reg - srcxsize_reg))) *( 2**transize))) 
                                                    : SRCADDR_UPDATED
                                                    :src_addr_reg + ((srcxsize_reg - src_left + 1) *( 2**transize)) ;
                    end
                    else
                        SRCADDR_UPDATED <=  src_addr_reg;
                end
                ...
                if(WREADY)
                    if(des_xaddr_inc == 1)
                    begin
 						DESADDR_UPDATED <= des_addr_reg + ((desxsize_reg - des_left + 1) * ( 2**transize));          
					end
                    else
                        DESADDR_UPDATED <=  des_addr_reg;
            end 
...
//        WSTRB <= 0;
        ...
        for(j=0;j<256;j=j+1) begin
            ...
            if ((disable_cmd_partsel && state == DONE_ST) || stop_cmd_apb )
            ENABLECMD_DATA    <= 1;
            else  
            ENABLECMD_DATA    <= 0;
...
//            WSTRB <= 0;
            case (state)
                ...
                begin
                    ...
                    reg1<=1;
                    reg2<=reg1;
                     // STAT_SRCTRIGINWAIT_DATA <= (use_src_trigin && reg1)?1'b1:0;
                   // STAT_DESTRIGINWAIT_DATA <= (use_des_trigin && reg1)?1'b1:0;
...
//                        end
                if (use_src_trigin && use_des_trigin) begin  
                   ...
                   if ((use_src_trigin && !((src_trigin_type == 2'b00 && src_trigin_sw)|| (src_trigin_type == 2'b10 && src_trigin))) && next_st == WAIT_TRIG) begin
                        STAT_SRCTRIGINWAIT_DATA <= 1'b1;
                    end
                    if ((use_des_trigin && !((des_trigin_type == 2'b00 && des_trigin_sw == 1)||(des_trigin_type == 2'b10 && des_trigin == 1)))  && next_st == WAIT_TRIG) begin
                        STAT_DESTRIGINWAIT_DATA <= 1'b1;
                    end
               end
...
//                    WSTRB <= WSTRB_COMB;
                   // WLAST <= (des_left == 1)? 1 : 0;
                    ...
                    if (WREADY && des_left > 0 && WVALID) begin
                      ...
                      WDATA     <=  !(WREADY && WLAST) ? ((fifo_mem[fifo_rptr+1] /*& strobe_mask*/) /*| (prev_WDATA & ~strobe_mask)*/): WDATA; 
                ...
                TRIG_OUT: begin
                    if (use_trigout && trigout_type == 'b10)begin
                        trig_out_req <= trig_out_ack ? 0 : 1; end
